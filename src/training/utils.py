...
def flatten_omegaconf(cfg: DictConfig) -> Dict[str, Any]:
    ...
    Python ``dict`` so legacy code that expects ``cfg['batch_size']`` keeps
    working after the Hydra migration.
    """
...
def get_cfg_dict(cfg: Any) -> Dict[str, Any]:
	"""
    Return a plain ``dict`` regardless of whether *cfg* is a DictConfig or a
    regular mapping.  Useful for unit tests that still pass in dicts.
    """

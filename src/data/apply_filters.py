#!/usr/bin/env python3
"""
Filter application module (Bilateral, Unsharp Mask, Laplacian, CLAHE, DStretch).
"""
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Apply contrast/denoise filters to train/val/test dataset.")
    ap.add_argument('--base_dir', required=True, help='Root dir with train/, val/, test/ splits')
    ap.add_argument('--output_dir', required=True, help='Destination root (will be created)')
    ap.add_argument('--filter_type', choices=fns.keys(), required=True, help='Which filter to apply')
    args = ap.parse_args()

    for subset in ['train', 'val', 'test']:
        process_subset(args.base_dir, args.output_dir, subset, fns[args.filter_type])

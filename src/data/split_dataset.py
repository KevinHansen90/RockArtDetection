#!/usr/bin/env python3
"""
Dataset splitting utility (train / val / test ratios or counts).
"""
...
def main():
    ...
    parser.add_argument("--output_dir", required=True, help="Directory where split subfolders (train/val/test) will be created.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility.")
    parser.add_argument("--image_ext", default=".jpg", help="Extension of the image files.")

    split_group = parser.add_mutually_exclusive_group(required=True)
    split_group.add_argument("--use_ratios", action="store_true", help="Split using ratios.")
    split_group.add_argument("--use_numbers", action="store_true", help="Split using specific numbers.")

    parser.add_argument("--train_ratio", type=float, default=0.75, help="Fraction for training.")
    parser.add_argument("--val_ratio", type=float, default=0.05, help="Fraction for validation.")
    parser.add_argument("--test_ratio", type=float, default=0.20, help="Fraction for testing.")

    parser.add_argument("--train_num", type=int, help="Number of images for training.")
    parser.add_argument("--val_num", type=int, help="Number of images for validation.")
    parser.add_argument("--test_num", type=int, help="Number of images for testing.")
    ...
    if ".jpg" in valid_exts and ".jpeg" not in valid_exts:
        valid_exts += (".jpeg",)
    ...
    if not all_images:
        print(f"Error: No images found with extension(s) '{','.join(valid_exts)}' in '{input_images_dir}'.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.use_ratios:
        ...
        if args.test_num is not None:
            ...
            if train_count + val_count + test_count > total_count:
                raise ValueError(f"Sum of train ({train_count}), val ({val_count}), and test ({test_count}) exceeds total ({total_count}).")
        else:
            test_count = total_count - train_count - val_count
            if test_count < 0:
                raise ValueError(f"Sum of train ({train_count}) and val ({val_count}) exceeds total ({total_count}).")

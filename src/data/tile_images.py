#!/usr/bin/env python3
"""
Tiling module for dividing large images into overlapping patches and mapping labels.
"""
...
def tile_image_and_labels_with_overlap(
    ...
    for y0 in range(0, H, step):
        for x0 in range(0, W, step):
            ...
            if is_partial and not allow_partial_tiles:
                continue
            ...
            if skip_empty_tiles and not tile_boxes:
                continue
...
def main():
    ...
    if not img_files:
        print(f"Error: No images found with extension(s) '{','.join(valid_exts)}' in '{args.input_images}'.", file=sys.stderr)
        sys.exit(1)

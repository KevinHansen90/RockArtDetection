...
│   │   ├── crop_motifs.py    # Extracts motifs from labeled images
│   │   └── cluster_motifs.py # Clustering script
│   ├── preprocessing/
...
Beyond object detection, this project explores unsupervised clustering to analyze visual patterns and potentially discover stylistic groupings within specific motif categories (e.g., "Animal"). This involves extracting motifs, generating feature representations using CNNs, and applying clustering algorithms.

### 1. Motif Extraction

**Script:** `src/preprocessing/crop_motifs.py`

* Extracts specific motifs (based on class ID) from the tiled dataset images using their bounding box labels.
* Optionally resizes the cropped motifs to a consistent dimension (based on the longer side) while maintaining aspect ratio. Resizing helps ensure that feature extraction is not biased by the original detected size of the motif.
* Saves each cropped motif as a separate image file (e.g., PNG) in an output directory.

**Example Command (Extracting 'Animal' class ID 0, resizing longer side to 224px):**

```bash
python src/preprocessing/crop_motifs.py \
    --images data/tiles/training/val/images \
    --labels data/tiles/training/val/labels \
...
**Output:** A directory (e.g., `data/clustering/cropped/`) containing individual image files for each detected and cropped animal motif from the input dataset split, resized for consistency. This process should be repeated for validation and/or test sets as needed.

### 2. Feature Extraction & Clustering

**Script:** `src/clustering/cluster_motifs.py`

* **Input:** Takes the directory of cropped (and potentially resized) motif images generated by `crop_motifs.py`.
* **Feature Extraction:**
    * Loads a pre-trained Convolutional Neural Network (CNN) specified by the user (`--feature-model`). Implemented options:
        * ***VGG16/VGG19:*** Good for texture and trace patterns.
        * ***ResNet*** (e.g., `resnet18`, `resnet50`): Robust, captures high-level shapes and details.
        * ***DenseNet*** (`densenet121`): Good feature reuse, potentially useful for subtle variations.
        * ***InceptionV3:*** Handles multi-scale features well.
    * Processes each motif image (resizing to fit model input, normalization) and extracts a feature vector from the penultimate layer.
* **Clustering:**
    * Applies an unsupervised clustering algorithm selected by the user (`--cluster-algo`) to the extracted feature vectors. Implemented algorithms:
        * ***K-Means:*** Simple, efficient baseline. Requires `--num-clusters` (K).
        * ***Agglomerative Clustering:*** Hierarchical, doesn't require K beforehand but needs it set for `sklearn` implementation (`--num-clusters`).
        * ***Spectral Clustering:*** Can handle non-convex cluster shapes. Requires `--num-clusters`.
        * ***DBSCAN:*** Density-based, does not require K but needs `--dbscan_eps` and `--dbscan_min_samples` (tuning required).
        * *(Note: Deep Embedded Clustering (DEC) mentioned in literature review is not implemented here due to its complexity).*
* **Output:**
    * Saves the clustering results to the directory specified by `--output`, potentially including:
        * A CSV file mapping motif filenames to assigned cluster IDs.
        * (Optional: `--copy-images`) Subdirectories for each cluster containing the corresponding motif images.
        * (Optional: `--visualize-tsne`) A *t-SNE* plot visualizing the clustered features.
* **Evaluation:** The quality of clustering can be assessed using metrics like *Silhouette Score* (internal coherence) or, if reference labels exist, *NMI*, *ARI*, or *Purity*. Qualitative evaluation by experts is crucial for interpreting the stylistic relevance of the resulting clusters.

**Example Command (Clustering cropped animals using ResNet50 features and K-Means with K=5):**

```bash
python src/clustering/cluster_motifs.py \
    --input data/clustering/cropped_animals_train_224px \
